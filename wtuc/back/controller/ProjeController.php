...
WHERE sicil_no=? ");
    ...
    function projeSayisi()
    {
        $sth = $this->conn->prepare("SELECT COUNT(*) AS projesayisi FROM projeler");
        $sth->execute();
        $result = $sth->fetch();
        return $result;
    }
    ...
    function projeleri_bitis_yaklasan()
    {
        $sth = $this->conn->prepare("SELECT *,projeler.id AS projeid, DATEDIFF(bitis_tarih,Date(Now())) AS kalanSure FROM projeler
INNER JOIN durum ON durum.id=projeler.durum_id
INNER JOIN calisma_turleri ON calisma_turleri.id=projeler.tur_id 
WHERE DATEDIFF(bitis_tarih,Date(Now()))<30 AND DATEDIFF(bitis_tarih,Date(Now()))>0 ORDER BY bitis_tarih ASC
");
        $sth->execute();
        $result = $sth->fetchAll();
        return $result;
