...
INNER JOIN departman ON departman.id=personel.departman_id ORDER BY personel.id ASC");
        $sth->execute();
        $result = $sth->fetchAll();
        return $result;
    }
...
INNER JOIN departman ON departman.id=personel.departman_id WHERE sicil_no=?");
    ...
    function personelSayisi(){
        $sth = $this->conn->prepare("SELECT COUNT(*) AS personelsayisi FROM personel");
        $sth->execute();
        $result = $sth->fetch();
        return $result;
    }

    function basvuruSayisi(){
        $sth = $this->conn->prepare("SELECT COUNT(*) AS basvurusayisi
FROM basvurular");
        $sth->execute();
        $result = $sth->fetch();
        return $result;
    ...
    function personelTur($departmanid){
        $sth = $this->conn->prepare("SELECT COUNT(*) AS personelTur
FROM personel WHERE departman_id = ?");
        $sth->execute([$departmanid]);
        $result = $sth->fetch();
        return $result;
    }

    function izindekiPersoneller(){
        $sth = $this->conn->prepare("SELECT personel.id as personelid,personel.ad,personel.telefon,personel.soyad,personel.resim,personel.sicil_no,personel.durum_id,durum.id,durum.durum,durum.renk,departman.departman FROM personel
INNER JOIN durum ON durum.id=personel.durum_id
INNER JOIN departman ON departman.id=personel.departman_id WHERE personel.durum_id=6 ORDER BY personel.id ASC");
        $sth->execute();
        $result = $sth->fetchAll();
        return $result;
    }
