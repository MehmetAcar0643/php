...
if (isset($_POST["izinTalep"])) {
    ...
    if((trim($_POST['bitis'])) && trim($_POST['baslangic'])&& ($_POST['bitis']<$_POST['baslangic'])){
        array_push($errorIzin, "İzin Başlangıcı, bitişinden sonra olamaz!");
    }
    if(trim($_POST['baslangic'])&& trim($_POST['baslangic'])< $bugunun_tarihi->format("Y-m-d")){
        array_push($errorIzin, "İzin Başlangıcı bugünden önce olamaz!");
    }
...
if (isset($_POST["odemeTalep"])) {
    ...
    if (empty(trim($_POST['miktar']))) {
        ...
        if($_POST['miktar']<0){
            array_push($errorOdeme, "İstenen miktar 0 dan küçük olamaz!");
        }else {
            $miktar = $_POST["miktar"];
        }
    }
